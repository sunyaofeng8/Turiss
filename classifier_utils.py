...
class ScoreProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "", "test_set.tsv")),
            "test")
...
class HelpProcessor(DataProcessor):
    """Processor for score prediction through comments"""
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            guid = self.process_text(line[0])
            text_a = tokenization.convert_to_unicode(line[5])
            label = tokenization.convert_to_unicode(1+int(5*float(line[3])/float(line[4])))
            #print(text_a)
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=None, label=label))
        return examples
